...
void print_game_state(const char *arts[], int gallows_state, const char *phrase, const char *eliminated){
    ...
    for (int i = 0; phrase[i] != '\0'; ++i) {
        ...
        if (strchr(punctuation, current_char) != NULL || string_contains_character(eliminated, current_char)) {
...
bool has_guessed_all(const char *secret, const char *guessed_letters){
    ...
    for (int i = 0; secret[i] != '\0'; ++i) {
        ...
        if (strchr(punctuation, current_char) == NULL && !string_contains_character(guessed_letters, current_char)) {
            return false;
        }
    }
    return true;
